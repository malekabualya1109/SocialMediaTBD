...
def get_embeddings(texts):
    ...
    with torch.no_grad():
        embeddings = model(**inputs).last_hidden_state.mean(dim=1) # get the embeddings of the documents
    return embeddings
...
def query_db(question, collection, top_k=3): # k is the number of the most relevant documents to return and add to the context window
    q_embeddings = get_embeddings([question]) # get the embeddings of the question to be used to query the database
    results = collection.query(query_embeddings=q_embeddings.numpy().tolist(), n_results=top_k) # query the database for the most relevant documents
...
model_name = "MaxyMin/llama-3.2-1B-Unsloth-FineTomeTrained-merged10"
...
messages = [
    ...
    {"role": "system", "content": "The person taklking to you is named " + prompt2},
...
# $ python3 LlamaRolePlay3.py "It is night time and you just saw Jake." "Jake" "What University do you go to?"
# Washington State University
# $ python3 LlamaRolePlay3.py "It is night time and you just saw Jake." "Jake" "What school do you go to?"
# Washington State University